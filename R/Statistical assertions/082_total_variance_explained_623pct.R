...
# Manuscript assertion: "62.3% of total variance"
...
risk_cols <- grep("^Q12\.13_", names(data), value = TRUE)[1:10]; risk_numeric <- apply(data[, risk_cols, drop=FALSE], 2, function(x) suppressWarnings(as.numeric(x))); if (!requireNamespace("psych", quietly=TRUE)) stop("psych required"); pca <- psych::principal(na.omit(risk_numeric), nfactors = 3, rotate = "varimax"); tot <- sum(pca$values[1:3]) / sum(pca$values) * 100; cat("Total variance %:", round(tot,2), "\n")
