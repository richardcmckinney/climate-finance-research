...
# Manuscript assertion: "KMO measure (.847)"
...
if (!requireNamespace("psych", quietly=TRUE)) stop("psych required"); risk_cols <- grep("^Q12\.13_", names(data), value = TRUE)[1:10]; risk_numeric <- apply(data[, risk_cols, drop=FALSE], 2, function(x) suppressWarnings(as.numeric(x))); print(psych::KMO(risk_numeric)$MSA)
