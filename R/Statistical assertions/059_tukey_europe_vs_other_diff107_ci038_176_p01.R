...
geo_risk_data <- na.omit(data.frame(region = factor(data$Q2.2), reg_risk = suppressWarnings(as.numeric(data$Q3.6_3)))); aov_geo <- aov(reg_risk ~ region, data = geo_risk_data); print(TukeyHSD(aov_geo)$region["2-4", ])
