...
# Manuscript assertion: "Bartlett's test (χ²(45)=2341.67, p<.001)"
...
risk_cols <- grep("^Q12\.13_", names(data), value = TRUE)[1:10]; risk_numeric <- apply(data[, risk_cols, drop=FALSE], 2, function(x) suppressWarnings(as.numeric(x))); if (!requireNamespace("psych", quietly=TRUE)) stop("psych required"); print(psych::cortest.bartlett(cor(risk_numeric, use="complete.obs"), n = nrow(na.omit(risk_numeric))))
